...
class WebSocketManager:
    def __init__(self):
        ...
        self.subscriptions = {}  # symbol -> list of websockets
    ...
    async def send_to_subscribers(self, symbol: str, data: dict):
        """Send data to all subscribers of a specific symbol"""
    ...
    def subscribe(self, websocket: WebSocket, symbol: str):
        ...
        if websocket not in self.subscriptions[symbol]:
            ...
            logger.info(f"Subscribed to {symbol}. Total subscribers: {len(self.subscriptions[symbol])}")
    ...
    def unsubscribe(self, websocket: WebSocket, symbol: str):
        ...
        if symbol in self.subscriptions and websocket in self.subscriptions[symbol]:
            ...
            logger.info(f"Unsubscribed from {symbol}. Total subscribers: {len(self.subscriptions[symbol])}")
            
    def get_subscription_count(self, symbol: str) -> int:
        """Get number of subscribers for a symbol"""
        return len(self.subscriptions.get(symbol, []))
